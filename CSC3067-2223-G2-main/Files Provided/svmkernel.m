...
case 'poly'
    ...
    % convert each cell array into a matric
%     for i=1:size(x,1)
%         b = x{i};
%         [row, col] = size(b);
%         sampleMatrix = reshape(b,row*col,1);
%         X{i} = sampleMatrix;
%     end

    %[row, col] = size(x{:,:});
    %sampleMatrix = reshape(x{:,:},row*col,1);


    
    ps= x *(xsup.*aux.^2)';
    % x = 124x1 Cell array (27x18 uint8)
    % xsup = 124x1 Cell array (27x18 uint8)
    % aux = 1
